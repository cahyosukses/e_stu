<?php
	...
	// master
	if (in_array($user['user_type_id'], array(USER_TYPE_ADMINISTRATOR, USER_TYPE_PRINCIPAL))) {
		$array_class_level = $this->class_level_model->get_array();
	} else {
		$array_class_level = $this->class_level_model->get_teacher_array(array( 'user_id' => $user['user_id'] ));
	}
	...
	$array_page['USER_TYPE_PRINCIPAL'] = USER_TYPE_PRINCIPAL;
...
<body class="centered-layout">
	...
	<section class="container">
		<section class="row-fluid">
			...
			<?php if (in_array($user['user_type_id'], array(USER_TYPE_TEACHER, USER_TYPE_PRINCIPAL, USER_TYPE_ADMINISTRATOR))) { ?>
...
$(document).ready(function() {
	...
	// selector
	if (Func.in_array(page.data.user.user_type_id, [page.data.USER_TYPE_TEACHER, page.data.USER_TYPE_PRINCIPAL, page.data.USER_TYPE_ADMINISTRATOR])) {
		// grid attendance
		...
		$('.btn-attendance-add').click(function() {
			if (page.is_valid()) {
				...
				// reset form
				var title = 'Attendance ' + page.data.format_date;
